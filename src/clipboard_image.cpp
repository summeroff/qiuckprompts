...
bool ClipboardHasImage()
{
    if (!OpenClipboardRetry(nullptr))
        return false;
    const bool has = IsClipboardFormatAvailable(CF_DIB) || IsClipboardFormatAvailable(CF_BITMAP) ||
...
bool ClipboardSaveImage(ClipboardImage& out, std::wstring* error)
{
    ...
    {
        if (error)
            *error = L"clipboard has no supported image format (need PNG or DIB)";
...
bool ClipboardRestoreImage(const ClipboardImage& img, std::wstring* error)
{
    ...
    {
        if (error)
            *error = L"SetClipboardData image failed";
        QP_LOG_ERROR(L"clipboard: restore image failed");
        return false;
    }
    QP_LOG_DEBUG(L"clipboard: restored image (png=%d dib=%d)", img.hasPng ? 1 : 0,
                 img.hasDib ? 1 : 0);
    return true;
}
