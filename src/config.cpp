...
UINT ParseVkToken(const std::wstring& tok) {
    if (tok.size() == 1) {
        ...
        if (c >= L'A' && c <= L'Z') return static_cast<UINT>(c);
...
bool ParseHotkey(const std::wstring& text, HotkeySpec& out, std::wstring* error) {
    ...
    while (start <= s.size()) {
        ...
        std::wstring part = Trim(plus == std::wstring::npos ? s.substr(start)
                                                            : s.substr(start, plus - start));
        const std::wstring pl = ToLower(part);
        if (pl == L"ctrl" || pl == L"control") mods |= MOD_CONTROL;
        else if (pl == L"alt") mods |= MOD_ALT;
        else if (pl == L"shift") mods |= MOD_SHIFT;
        else if (pl == L"win" || pl == L"windows" || pl == L"meta") mods |= MOD_WIN;
        else {
            vk = ParseVkToken(part);
            if (!vk) {
                if (error) *error = L"unknown hotkey token: " + part;
                return false;
            }
        }
    ...
    if (!vk) {
        if (error) *error = L"hotkey missing key: " + text;
        return false;
    }
...
std::wstring BuildPromptPayload(const std::wstring& promptTemplate,
    ...
    if (editorText.empty()) return prompt;
...
void GetBuiltinBindings(std::vector<HotkeyBinding>& out) {
    ...
    auto add = [&](wchar_t key, const wchar_t* name, const wchar_t* label,
                   const wchar_t* service, const wchar_t* url, const wchar_t* hint,
                   const wchar_t* prompt, bool capture, bool image) {
        HotkeyBinding b;
    ...
    add(L'J', L"grammar_meta", L"Grammar quick (Meta)", L"meta",
    ...
    add(L'K', L"fact_gemini", L"Deep fact-check (Gemini)", L"gemini",
        ...
        true, false);
    add(L'L', L"idea_grok", L"Idea collab (Grok)", L"grok",
        L"https://grok.com/", L"Grok",
    ...
    add(L'O', L"grammar_context_meta", L"Grammar in context (Meta)", L"meta",
        ...
        L"My message:\n```\n{{TEXT}}\n```\n\nOptional context:\n```\n{{CONTEXT}}\n```\n",
        true, false);
    add(L'I', L"screenshot_meta", L"Screenshot review (Meta)", L"meta",
        L"https://www.meta.ai/", L"Meta",
...
bool LoadConfigFile(const std::wstring& pathOrEmpty, AppConfig& cfg, std::wstring* error) {
    ...
    if (path.empty()) {
        ...
        if (FileExists(a)) path = a;
        else if (FileExists(b)) path = b;
        else {
            if (error) *error = L"no config file (tried config/qiuckprompts.ini)";
            return false;
        }
    }
    if (!FileExists(path)) {
        if (error) *error = L"config not found: " + path;
        return false;
    }
    ...
    for (const auto& secPair : secs) {
        ...
        if (name.empty() || name == L"settings") continue;
        ...
        if (!ParseHotkey(hk, b.hotkey, &herr)) {
            QP_LOG_ERROR(L"config: [%s] bad hotkey '%s': %s", name.c_str(), hk.c_str(), herr.c_str());
            continue;
        }

        b.captureEditor = get(L"capture_editor") != L"0";
        b.requireClipboardImage =
            get(L"require_clipboard_image") == L"1" ||
            get(L"require_clipboard_image") == L"true";
        if (b.requireClipboardImage) b.captureEditor = false;
        ...
        QP_LOG_INFO(L"config: binding [%s] %s service=%s url=%s image=%d capture=%d prompt=%zu wchar",
                    b.name.c_str(), b.hotkey.display.c_str(), b.service.c_str(),
                    b.aiUrl.c_str(), b.requireClipboardImage ? 1 : 0,
                    b.captureEditor ? 1 : 0, b.promptBody.size());
        cfg.bindings.push_back(std::move(b));
    }

    if (cfg.bindings.empty()) {
        if (error) *error = L"config loaded but no valid bindings: " + path;
        return false;
    }
    return true;
}
...
bool ParseCommandLine(int argc, wchar_t** argv, AppConfig& cfg, std::wstring* error) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == L"--console") { cfg.console = true; continue; }
        if (arg == L"--self-test" || arg == L"--help" || arg == L"-h" || arg == L"/?") continue;
        if (arg == L"--insert-only") { cfg.forceInsertOnly = true; continue; }
        if (arg == L"--no-uia") { cfg.workflow.pageReadyUseUia = false; continue; }
        if (arg == L"--hotkey-on-press") { cfg.hotkeyTrigger = HotkeyTriggerMode::OnPress; continue; }
        if (arg == L"--hotkey-on-release") { cfg.hotkeyTrigger = HotkeyTriggerMode::OnRelease; continue; }
