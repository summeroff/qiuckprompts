...
bool HotkeyManager::RegisterAll(HWND hwnd, std::vector<HotkeyBinding> bindings,
    ...
    for (auto& b : bindings) {
        ...
        QP_LOG_INFO(L"hotkey registered id=%d %s -> %s (%s) trigger=%s",
                    b.id,
...
bool HotkeyManager::IsChordReleased(const HotkeyBinding& b) const {
    ...
    if ((m & MOD_CONTROL) && AnyControlDown()) return false;
    if ((m & MOD_ALT)     && AnyAltDown())     return false;
    if ((m & MOD_SHIFT)   && AnyShiftDown())   return false;
    if ((m & MOD_WIN)     && AnyWinDown())     return false;
...
void HotkeyManager::StartReleaseTimer() {
    ...
    SetTimer(hwnd_, kReleaseTimerId,
             static_cast<UINT>(releasePollMs_ > 0 ? releasePollMs_ : 15),
             nullptr);
}
...
void HotkeyManager::ClearPending(const wchar_t* reason) {
    ...
    QP_LOG_DEBUG(L"hotkey: clear pending id=%d (%s)", pendingId_,
                 reason ? reason : L"");
    pendingId_ = 0;
    pendingSince_ = 0;
    StopReleaseTimer();
...
void HotkeyManager::FirePending(const wchar_t* reason) {
    ...
    if (busy_) {
        QP_LOG_WARN(L"hotkey: fire suppressed (busy) id=%d %s",
                    id, b->hotkey.display.c_str());
        return;
    }

    const DWORD held = pendingSince_ ? (GetTickCount() - pendingSince_) : 0;
    QP_LOG_INFO(L"hotkey: FIRE id=%d %s after %s (held ~%lu ms)",
                id, b->hotkey.display.c_str(),
                reason ? reason : L"release",
...
void HotkeyManager::ArmPending(int id) {
    ...
    if (busy_) {
        QP_LOG_WARN(L"hotkey: ignore arm (busy) id=%d %s",
                    id, b->hotkey.display.c_str());
        return;
    }
    ...
    QP_LOG_DEBUG(L"hotkey: ARMED id=%d %s — waiting for release (timeout %d ms)",
                 id, b->hotkey.display.c_str(), releaseTimeoutMs_);
}
...
void HotkeyManager::OnWmHotkey(WPARAM wParam) {
    ...
    QP_LOG_DEBUG(L"WM_HOTKEY id=%d %s -> %s (mode=%s busy=%d)",
                 id, b->hotkey.display.c_str(), b->templateId.c_str(),
                 HotkeyTriggerModeName(triggerMode_), busy_ ? 1 : 0);
...
void HotkeyManager::OnTimer(WPARAM timerId) {
    if (timerId != kReleaseTimerId) return;
    if (pendingId_ == 0) {
        StopReleaseTimer();
        return;
    }
    ...
    if (static_cast<int>(elapsed) >= releaseTimeoutMs_) {
        ...
        QP_LOG_WARN(L"hotkey: release timeout (%lu ms) id=%d %s — firing anyway",
                    ...
                    pendingId_, b->hotkey.display.c_str());
