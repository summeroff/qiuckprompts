...
bool SendKeyChord(UINT modifiers, WORD vk, std::wstring* error)
{
    ...
    auto pushDown = [&](WORD k) {
        seq.push_back(MakeVk(k, false));
    };
    auto pushUp = [&](WORD k) {
        seq.push_back(MakeVk(k, true));
    };
    ...
    QP_LOG_DEBUG(L"input: chord mods=0x%X vk=0x%02X (%zu events)", modifiers, vk, seq.size());
    return SendInputs(seq, error);
}

bool SendCopy(std::wstring* error)
{
    return SendKeyChord(MOD_CONTROL, 'C', error);
}
bool SendPaste(std::wstring* error)
{
    return SendKeyChord(MOD_CONTROL, 'V', error);
}
bool SendSelectAll(std::wstring* error)
{
    return SendKeyChord(MOD_CONTROL, 'A', error);
}
bool SendNewTab(std::wstring* error)
{
    return SendKeyChord(MOD_CONTROL, 'T', error);
}
bool SendFocusOmnibox(std::wstring* error)
{
    return SendKeyChord(MOD_CONTROL, 'L', error);
}
...
bool SendUnicodeText(const std::wstring& text, std::wstring* error)
{
    ...
    QP_LOG_DEBUG(L"input: unicode %zu wchar (%zu newlines as Enter, %zu events)", text.size(),
                 newlines, inputs.size());
...
bool ClipboardReadUnicode(std::wstring& out, std::wstring* error)
{
    ...
    {
        if (error)
            *error = L"OpenClipboard read failed: " + LastErrorMessage();
        return false;
    }
...
bool ClipboardWriteUnicode(const std::wstring& text, std::wstring* error)
{
    ...
    {
        if (error)
            *error = L"OpenClipboard write failed: " + LastErrorMessage();
        return false;
    }
    if (!EmptyClipboard())
    {
        CloseClipboard();
        if (error)
            *error = L"EmptyClipboard failed: " + LastErrorMessage();
        return false;
    }
    ...
    if (!SetClipboardData(CF_UNICODETEXT, mem))
    {
        ...
        CloseClipboard();
        if (error)
            *error = L"SetClipboardData failed: " + LastErrorMessage();
        return false;
    }
    CloseClipboard();
...
bool ForceForeground(HWND hwnd, std::wstring* error)
{
    ...
    QP_LOG_DEBUG(L"focus: ForceForeground hwnd=%p ok=%d fgNow=%p SetFG=%d", hwnd, success ? 1 : 0,
                 now, ok ? 1 : 0);
    if (!success && error)
    {
...
FocusSnapshot CaptureFocusSnapshot()
{
    ...
    QP_LOG_DEBUG(L"focus: fg=%p '%s' class='%s' pid=%lu focus=%p class='%s'", info.foreground,
                 info.fgTitle.c_str(), info.fgClass.c_str(), static_cast<unsigned long>(info.pid),
