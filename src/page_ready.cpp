...
struct ComInit {
    ...
    ComInit() {
        ...
        if (hr == RPC_E_CHANGED_MODE) {
            ...
            QP_LOG_ERROR(L"com: CoInitializeEx failed hr=0x%08X",
...
bool LooksLikeNewTabTitle(const std::wstring& title) {
    ...
    if (l == L"new tab" || l == L"untitled" || l == L"about:blank") return true;
    if (l.find(L"new tab") != std::wstring::npos) return true;
    // Just the browser chrome name with no page title yet
    if (l == L"google chrome" || l == L"chrome beta" || l == L"microsoft edge" ||
        l == L"chromium") {
        return true;
    }
    return false;
...
bool IsOmniboxName(const std::wstring& name) {
    ...
    if (l.find(L"address") != std::wstring::npos) return true;
    if (l.find(L"search bar") != std::wstring::npos) return true;
    if (l.find(L"omnibox") != std::wstring::npos) return true;
    if (l.find(L"find in page") != std::wstring::npos) return true;
    if (l.find(L"find bar") != std::wstring::npos) return true;
...
bool IsPromisingEdit(IUIAutomationElement* el, std::wstring* nameOut) {
    ...
    if (!isEdit && !isDoc) return false;

    if (!GetBoolProp(el, UIA_IsEnabledPropertyId, false)) return false;
    // Offscreen edits are usually hidden chrome / inactive tabs
    if (GetBoolProp(el, UIA_IsOffscreenPropertyId, true)) return false;

    const std::wstring name = GetStringProp(el, UIA_NamePropertyId);
    if (IsOmniboxName(name)) return false;
...
IUIAutomation* CreateAutomation(std::wstring* error) {
    if (!Com().ok) {
        if (error) *error = L"COM not initialized";
        return nullptr;
    }
...
bool TryFocusedEdit(IUIAutomation* automation, bool setFocus,
                    std::wstring* editName, std::wstring* error) {
    IUIAutomationElement* focused = nullptr;
    HRESULT hr = automation->GetFocusedElement(&focused);
    if (FAILED(hr) || !focused) {
        if (error) *error = L"GetFocusedElement failed";
        return false;
    }
...
bool TryFindEditInTree(IUIAutomation* automation, HWND hwnd, bool setFocus,
                       std::wstring* editName, std::wstring* error) {
    IUIAutomationElement* root = nullptr;
    HRESULT hr = automation->ElementFromHandle(hwnd, &root);
    if (FAILED(hr) || !root) {
        if (error) *error = L"ElementFromHandle failed";
        return false;
    }
    ...
    if (!edit) {
        if (error) *error = L"no suitable Edit/Document in UIA tree";
        return false;
    }

    std::wstring name = editName ? *editName : GetStringProp(edit, UIA_NamePropertyId);
    if (editName && editName->empty()) *editName = name;

    if (setFocus) {
        const HRESULT fhr = edit->SetFocus();
        QP_LOG_DEBUG(L"uia: SetFocus on edit name='%s' hr=0x%08X",
                     name.c_str(), static_cast<unsigned>(fhr));
...
bool TitleLooksReady(const std::wstring& title, const std::wstring& hint) {
    if (LooksLikeNewTabTitle(title)) return false;
    if (!hint.empty() && !ContainsI(title, hint)) return false;
    return true;
}
...
std::wstring TitleHintFromUrl(const std::wstring& url) {
    ...
    if (u.find(L"meta.ai") != std::wstring::npos) return L"Meta";
    if (u.find(L"gemini.google") != std::wstring::npos) return L"Gemini";
    if (u.find(L"chatgpt.com") != std::wstring::npos ||
        u.find(L"chat.openai.com") != std::wstring::npos) return L"ChatGPT";
    if (u.find(L"claude.ai") != std::wstring::npos) return L"Claude";
    if (u.find(L"grok.x.ai") != std::wstring::npos ||
        u.find(L"grok.com") != std::wstring::npos) return L"Grok";
    if (u.find(L"copilot.microsoft") != std::wstring::npos) return L"Copilot";
    if (u.find(L"perplexity.ai") != std::wstring::npos) return L"Perplexity";
    return {};
}

bool UiaFindChatEdit(HWND browserHwnd, std::wstring* editName,
                     bool setFocus, std::wstring* error) {
    if (!browserHwnd || !IsWindow(browserHwnd)) {
        if (error) *error = L"bad browser hwnd";
        return false;
    }
...
bool WaitForAiPageReady(const PageReadyConfig& cfg, PageReadyResult& out,
    ...
    if (!cfg.browserHwnd || !IsWindow(cfg.browserHwnd)) {
        if (error) *error = L"WaitForAiPageReady: invalid browser hwnd";
        return false;
    }
    ...
    QP_LOG_INFO(L"page_ready: wait start timeout=%dms min=%dms hint='%s' uia=%d hwnd=%p",
                cfg.timeoutMs, cfg.minWaitMs, cfg.titleHint.c_str(),
                cfg.useUia ? 1 : 0, cfg.browserHwnd);
    ...
    if (cfg.useUia) {
        ...
        if (!automation) {
            QP_LOG_WARN(L"page_ready: UIA unavailable (%s) — title-only fallback",
                        uerr.c_str());
        }
    }
    ...
    for (;;) {
        ...
        if (!IsWindow(cfg.browserHwnd)) {
            ...
            if (error) *error = L"browser window disappeared";
            return false;
        }
        ...
        if (titleReady && automation) {
            ...
            if (cfg.preferFocusedEdit &&
                ...
                if (TryFindEditInTree(automation, cfg.browserHwnd,
                                      cfg.focusFoundEdit, &name, nullptr)) {
                    editReady = true;
                    out.usedUia = true;
                    out.editName = name;
                }
        ...
        if ((elapsed % 1000) < cfg.pollMs) {
            QP_LOG_DEBUG(L"page_ready: t=%dms titleReady=%d editReady=%d title='%s'",
                         elapsed, titleReady ? 1 : 0, editReady ? 1 : 0,
                         lastTitle.c_str());
        }
    ...
    if (out.detail.empty()) {
        if (out.usedUia) {
            out.detail = out.focusedEdit ? L"focused UIA edit"
                                         : L"found UIA edit in tree";
        } else {
            out.detail = L"title heuristic only";
        }
    }
