...
bool LooksBrowserClass(const std::wstring& cls) {
    return cls == L"Chrome_WidgetWin_1"
        ...
        || cls == L"ApplicationFrameWindow"; // Edge/Store sometimes
}

void AppendTitlesFile(const std::wstring& line) {
    std::lock_guard<std::mutex> lock(g_mu);
    if (g_titlesLogPath.empty()) return;
...
std::wstring FormatSample(const wchar_t* where, HWND hwnd,
    ...
    wchar_t head[160];
    swprintf(head, 160,
             L"TITLE_SAMPLE ts=%04u-%02u-%02uT%02u:%02u:%02u.%03u where=%s",
    ...
    wchar_t mid[128];
    swprintf(mid, 128, L" hwnd=%p fg=%d pid=%lu",
             hwnd, isFg ? 1 : 0, static_cast<unsigned long>(pid));
    line += mid;
    line += L" class='";
    line += cls;
    line += L"' exe='";
...
BOOL CALLBACK SweepProc(HWND hwnd, LPARAM lp) {
    ...
    const bool browserExe =
        exeL == L"chrome.exe" || exeL == L"msedge.exe" ||
        exeL == L"brave.exe" || exeL == L"firefox.exe";
    const bool browserCls = LooksBrowserClass(cls);

    if (!browserExe && !browserCls) return TRUE;
...
void LogBrowserTitleSweep(const wchar_t* where) {
    ...
    QP_LOG_INFO(L"TITLE_SAMPLE sweep end where=%s browsers=%d",
                where ? where : L"?", ctx.count);
}
